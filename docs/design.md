...
- 多种预设模板 + 自定义模板
- 批量生成和导出功能
- **v2.0 新增**：多卡片分页编辑、画布缩放、面板折叠、元素等比例缩放、背景图适配、字体导入
...
interface Element {
  ...
  height: number;
  rotation: number;
  scale: number;           // 元素缩放比例（v2.0 新增）
  maintainAspectRatio: boolean; // 约束比例（v2.0 新增）
  zIndex: number;
  content?: string;
  src?: string;
  style?: TextStyle;
